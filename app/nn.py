...
class autoencoder(nn.Module):
	...
	def forward(self, data):
		...
		result = fun.sigmoid(self.fc2(active));
		return result;
	...
	def cost(self, data):
		...
		loss += self.decay * (self.fc2.weight ** 2).sum();
	...
	def train(self, data, lr = 1, max_iter = 400):
		...
		def closure():
			...
			print("count: %d, time: %f, loss: %f" % (count, duration, loss.data.item()));
			return loss;
...
class stacked_autoencoder(nn.Module):
	def __init__(self, size, size_class, decay, alpha, beta, theta_decay):
		...
		self.theta = nn.Linear(size[-1], size_class, bias = False);
		self.decay = decay;
		self.alpha = alpha;
		self.beta = beta;
	...
	def forward(self, data):
		...
		result = fun.softmax(result, dim = 1);
		return result;
